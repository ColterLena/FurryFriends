...
namespace FurryFriends.Controllers
{
    ...
    [ApiController]
    public class DogWalkerSessionsController : ControllerBase
    {
        ...
        public DogWalkerSessionsController(DatabaseContext context, IConfiguration config)
        {
            _context = context;
        ...
        {
            var foundUser = await _context.DogWalkers.FirstOrDefaultAsync(user => user.Email == loginUser.Email);
            ...
            if (foundUser != null && foundUser.IsValidPassword(loginUser.Password))
            {
                ...
                var response = new
                {
                    ...
                    token = new TokenGeneratorForClientSessions(JWT_KEY).TokenFor(foundUser),
